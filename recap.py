...
# Check ALL_SECRETS first (for cloud platforms), fallback to standard os.environ
...
def process_chapter(chapter_url):
    ...
    # 6. AI Scripting using 1 Big API Call & Enforced Structured JSON Output
    print(f"[{chap_num}] AI Writing Script (Structured Output)...")
    prompt = (
        ...
        "Generate a dynamic narration. For 'panels', specify image_index, start_mark, end_mark, narration text, and effect."
    )
                    ...
                    "panels": {
                        ...
                        "items": {
                            ...
                            "properties": {
                                ...
                                "start_mark": {"type": "INTEGER"},
                                "end_mark": {"type": "INTEGER"},
    ...
    # 7. Batch Voice Generation
    print(f"[{chap_num}] Batch Generating Voices in Parallel...")
    audio_results = {}
    
    def generate_voice(idx, text):
        try:
            samples, _ = kokoro.create(text, voice=VOICE_MODEL, speed=AUDIO_SPEED, lang="en-us")
            return idx, samples
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=os.cpu_count() or 4) as executor:
        ...
        for i, block in enumerate(script_data.get('panels',[])):
            if block.get('image_index') not in original_files: continue
            
            narration_text = str(block.get('narration', ''))
            narration_text = re.sub(r'[\n\r]+', ' ', narration_text)
            narration_text = re.sub(r'\s+', ' ', narration_text).strip()
            if not narration_text: narration_text = "..."
            
            voice_tasks.append(executor.submit(generate_voice, i, narration_text))
            
        for future in concurrent.futures.as_completed(voice_tasks):
            idx, samples = future.result()
            audio_results[idx] = samples
    ...
    for i, block in enumerate(script_data.get('panels',[])):
        ...
        # Retrieve pre-generated audio array from batch
        samples = audio_results.get(i, np.zeros(24000))
        ...
        # --- EFFECT LOGIC WITH ENLARGED FULL-SCREEN CENTERING ---
...
def stitch_all_chapters(manga_name):
...
stitch_all_chapters(target_manga_name)
